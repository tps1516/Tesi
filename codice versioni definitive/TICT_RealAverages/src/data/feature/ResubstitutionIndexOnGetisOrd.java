...
public class ResubstitutionIndexOnGetisOrd implements AutocorrelationI {
	...
	public ResubstitutionIndexOnGetisOrd(SnapshotData data, Feature feature,
		...
		for (int i = beginIndex; i <= endIndex; i++) {
			...
			NumericValue Xi = (NumericValue) (data.getSensorPoint(i)
					.getMeasure(featureIndex));
			if (!Xi.isNull()) {
				N++;
	...
	{
		NumericFeature f = (NumericFeature) feature;
		NumericValue X = (NumericValue) trainingSet.getSensorPoint(end + 1)
				.getMeasure(feature.getIndexMining());
		if (!X.isNull()) {
	...
	{
		NumericFeature f = (NumericFeature) feature;
		NumericValue X = (NumericValue) trainingSet.getSensorPoint(begin - 1)
				.getMeasure(feature.getIndexMining());
		if (!X.isNull()) {
			double giValue = X.getGetis();
			sumX -= giValue;
	...
	public Double compute(SnapshotData data, Feature feature, SnapshotWeigth W,
		...
		for (int i = beginIndex; i <= endIndex; i++) {
			...
			int id1 = data.getSensorPoint(i).getId();
			NumericValue Xi = (NumericValue) data.getSensorPoint(i).getMeasure(
					featureIndex);
			if (!Xi.isNull()) {
				N++;
