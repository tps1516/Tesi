...
public class NumericFeature extends Feature implements Cloneable {
	...
	// private Double streamMean=new Double(0.0); // modello sulla landmark
	// window
	...
	public void computeAutocorrelation(AutocorrelationI a, SnapshotData data,
		...
		// autocorrelationMeasure=a.compute(data, this, W, beginIndex,
		// endIndex);

		if (a instanceof ResubstitutionIndexOnGetisOrd) {
			autocorrelationMeasure = new ResubstitutionIndexOnGetisOrd(data,
					this, W, beginIndex, endIndex).get(); // Annalisa per
															// ottimizzazione di
															// ResubstitutionIndexOnGetisAndOrd
			// System.out.println("Auto feature "+a.compute(data, this, W,
			// beginIndex, endIndex)+ " " +autocorrelationMeasure);
		} else if (a instanceof ResubstitutionIndex) {
			autocorrelationMeasure = new ResubstitutionIndex(data, this, W,
					beginIndex, endIndex).get(); // Annalisa per ottimizzazione
													// di
													// ResubstitutionIndexOnGetisAndOrd
			// System.out.println("Auto feature "+a.compute(data, this, W,
			// beginIndex, endIndex)+ " " +autocorrelationMeasure);
