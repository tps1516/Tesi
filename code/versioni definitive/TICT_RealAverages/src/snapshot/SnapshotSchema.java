...
public class SnapshotSchema implements Serializable, Cloneable {
	...
	public SnapshotSchema(String iniFile) throws IOException,
		...
		// Update mining index key, spatial, spatial,..., spatial, target,
		// target, ..., target
		int id = 0;
	...
	public void reset() {
		...
		for (Feature fs : target) {
			if (!fs.getStopTree()) // altrimenti Mantengo l'ultimo modello
									// costruito
				fs.clear();
