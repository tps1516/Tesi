...
public class SnapshotData implements Iterable<SensorPoint> {
	...
	private SensorPoint sensorSmoothing(SnapshotSchema schema,
			SensorPoint realSensorPoint, SensorPoint smootSensorPoint,
			Double alpha) {

		SensorPoint res = new SensorPoint(realSensorPoint.getId());

		for (Feature f : schema.getSpatialList()) {
			double value = ((double) realSensorPoint.getMeasure(
					f.getIndexMining()).getValue());
			NumericValue nv = new NumericValue(value, f.getIndexMining());
			res.addMeasure((Value) nv);
		}

		for (Feature f : schema.getTargetList()) {
			Double realValue;

			realValue = ((Double) realSensorPoint
					.getMeasure(f.getIndexMining()).getValue());

			Double smootValue = ((Double) smootSensorPoint.getMeasure(
					f.getIndexMining()).getValue());

			Double value = (realValue * alpha) + (smootValue * (1 - alpha));
			NumericValue nv = new NumericValue(value, f.getIndexMining());
			res.addMeasure((Value) nv);
		}
		return res;
	...
	public SnapshotData computeSmoothing(Double alpha, SnapshotData snapSmoot,
			SnapshotSchema schema) {

		ArrayList<SensorPoint> resultSensorsPoint = new ArrayList<SensorPoint>();

		if (snapSmoot == null) {
			for (SensorPoint sp : this) {
				resultSensorsPoint.add((SensorPoint) sp.clone());
			}

			return new SnapshotData(resultSensorsPoint);
		}
		snapSmoot.sort();
		ArrayList<SensorPoint> sensorsSmoot = snapSmoot.getData();
		boolean flag = true;

		int i = 0;

		for (SensorPoint realSensorPoint : this) {

			/*
			 * se l'indice i è diverso dal numero di sensori nello snapSmooting
			 * posso continuare a cercare i sensori reali in quelli nello
			 * snapshotSmoothing
			 */
			if (i != sensorsSmoot.size()) {

				if (realSensorPoint.getId() == sensorsSmoot.get(i).getId()) {

					/*
					 * L'id del sensore reale corrisponde con l'id del sensore
					 * predetto con smoothing all'istante precedente posso
					 * procedere per tale sensore quindi a calcolare il suo
					 * corrispondente smoothing e ad aggiungerlo alla lista di
					 * sensori per creare lo snapshot da restituire. Infine
					 * incremento l'indice
					 */
					SensorPoint sensorRes = sensorSmoothing(schema,
							realSensorPoint, sensorsSmoot.get(i), alpha);
					resultSensorsPoint.add(sensorRes);
					i = i + 1;

				} else if (realSensorPoint.getId() < sensorsSmoot.get(i)
						.getId()) {

					/*
					 * gli id non corrispondevano e sicuramente il sensore reale
					 * non è presente nello snapshotSmoothing inserisco il
					 * sensore reale nella lista di sensori cos'ì com'è per esso
					 * ricomincia l'operazione di smoothing
					 */
					resultSensorsPoint.add((SensorPoint) realSensorPoint
							.clone());

				} else {

					/*
					 * in questo caso gli id non corrispondevano ma c'è
					 * possibilità che possa essere presente in posizioni
					 * successive procedo a cercarlo.
					 */

					flag = false;

					while (!flag & i != sensorsSmoot.size() - 1) {
						i = i + 1;
						if (realSensorPoint.getId() == sensorsSmoot.get(i)
								.getId()) {
							SensorPoint sensorRes = sensorSmoothing(schema,
									realSensorPoint, sensorsSmoot.get(i), alpha);
							resultSensorsPoint.add(sensorRes);
							i = i + 1;
							flag = true;
						} else if (realSensorPoint.getId() < sensorsSmoot
								.get(i).getId()) {
							resultSensorsPoint
									.add((SensorPoint) realSensorPoint.clone());
							flag = true;
						}
				...
				/*
				 * l'indice ha raggiunto il numero di sensori nello
				 * snapshotSmoothing quindi gli eventuali sensori reali rimasti
				 * devono essere aggiunti così come arrivano quindi per essi
				 * ricomincia l'operazione di smoothing
				 */
				resultSensorsPoint.add((SensorPoint) realSensorPoint.clone());
