...
public abstract class Node implements Serializable {
	...
	void update(SnapshotData trainingSet, AutocorrelationI a, SnapshotWeigth W,
		...
		for (Feature f : this.getSchema().getTargetList()) {
					...
					// (father)
			{
						...
						&& father
								...
								.get(f.getIndexMining()
										- father.getSchema().getSpatialList()
												.size()).getStopTree())
				// la feature era foglia già al passo precedente
				{
				...
				// è diventato foglia a questo passo oppure il nodo è la radice
				// dell'albero
				// quindi: father==null
				else {
					f.clear();
		...
		for (Feature f : this.getSchema().getTargetList()) {
			...
			if (!f.getStopTree()) // non sono foglie
				f.computeAutocorrelation(a, trainingSet, W, beginExampleIndex,
						endExampleIndex);
			else // ma non lo erano prima
			if (father == null
	...
	void sampling(SnapshotData data, String centroidType, float centroidPerc) {
		...
		if (centroidType.equals("random"))
			centroid = data.sampleCentroid(schema, beginExampleIndex,
					endExampleIndex, centroidPerc);
		else
			centroid = data.quadSampleCentroid(schema, beginExampleIndex,
					endExampleIndex, mbr, centroidPerc);

	}
	
	public FeatureMeanNode getavg(){
		return this.avg;
	}
