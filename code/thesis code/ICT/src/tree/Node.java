...
public abstract class Node implements Serializable{
	...
	public Node( SnapshotData trainingSet, SnapshotSchema schema, int beginExampleIndex, int endExampleIndex, int minExamples, int depth, Node father){
		this.beginExampleIndex=beginExampleIndex;
		this.endExampleIndex=endExampleIndex;
		this.setSchema((SnapshotSchema)schema.clone());
		this.father=father;
		...
		for(int i=beginExampleIndex;i<=endExampleIndex;i++)
		{
			for(SpatialFeature sf:this.getSchema().getSpatialList()){
				 ...
				 sf.setMax((Double)trainingSet.getSensorPoint(i).getMeasure(index).getValue()); // update max
				 sf.setMin((Double)trainingSet.getSensorPoint(i).getMeasure(index).getValue()); //update min
				 sf.setMean((Double)trainingSet.getSensorPoint(i).getMeasure(index).getValue());; //update mean
		...
		// Compute Autocorrelation Measure over current Target Attributes and Identify the Target Leaf
	
		for(Feature f:this.getSchema().getTargetListNotLeaf()){
			//	System.out.println("Evaluating ..."+ f.getName());
			if(f instanceof CategoricalFeature) 
				//f.computeAutocorrelation(new Entropy(), trainingSet, null, beginExampleIndex, endExampleIndex);
				throw new RuntimeException("Unsupported categorical features in optimized version");
			else
				f.computeAutocorrelation(new ResubstitutionIndex(), trainingSet, null, beginExampleIndex, endExampleIndex);
	...
	/*public Node(AutocorrelationI a, SnapshotData trainingSet, SnapshotSchema schema, SnapshotWeigth W, int beginExampleIndex, int endExampleIndex, int minExamples, int depth, Node father, boolean temp){
		this.beginExampleIndex=beginExampleIndex;
		this.endExampleIndex=endExampleIndex;
		this.setSchema((SnapshotSchema)schema.clone());
		this.father=father;
		...
		for(Feature f:this.getSchema().getTargetListNotLeaf()){
			//	System.out.println("Evaluating ..."+ f.getName());
				if(a!=null){
					f.computeAutocorrelation(a, trainingSet, W, beginExampleIndex, endExampleIndex);
...
*/
	...
	public Node(AutocorrelationI a, SnapshotData trainingSet, SnapshotSchema schema, SnapshotWeigth W, int beginExampleIndex, int endExampleIndex, int minExamples, int depth, Node father){
		this.beginExampleIndex=beginExampleIndex;
		this.endExampleIndex=endExampleIndex;
		this.setSchema((SnapshotSchema)schema.clone());
		this.father=father;
	...
	//Annalisa Added for optimization
		...
		public Node(Node n, AutocorrelationI a, SnapshotData trainingSet, SnapshotSchema schema, SnapshotWeigth W, int beginExampleIndex, int endExampleIndex, int minExamples, int depth, Node father){
			this.beginExampleIndex=beginExampleIndex;
			this.endExampleIndex=endExampleIndex;
			this.setSchema((SnapshotSchema)schema.clone());
			this.father=father;
			...
			for(Feature f:this.getSchema().getTargetList()){
				 if(!f.getStopTree()){
					 ...
					 if(f instanceof NumericFeature) {
						 ((NumericFeature) f).setMax(((NumericFeature)(n.getSchema().getTargetList().get(i))).getMax());
						 ((NumericFeature) f).setMin(((NumericFeature)(n.getSchema().getTargetList().get(i))).getMin());
						 ((NumericFeature) f).setMean(((NumericFeature)(n.getSchema().getTargetList().get(i))).getMean());
					 }
			...
			for(Feature f:this.getSchema().getTargetListNotLeaf()){
				//	System.out.println("Evaluating ..."+ f.getName());
					if(a!=null)
						f.setAutocorrelation(n.getSchema().getTargetListNotLeaf().get(i).getAutocorrelation());
					i++;
	...
	double getHeuristic(Feature f){
		int index=f.getIndexMining();
		return getSchema().getTargetList().get(index-getSchema().getSpatialList().size()).getAutocorrelation();
	}
	...
	public String toString(){
		...
		for(Feature f:getSchema().getTargetList()){
			//str+=f.getName()+"=(asp)"+f.getPrototype(a)+"[Stream:"+f.getStreamPrototype(a)+"](spat)"+f.getPrototype(s)+"[Stream:"+f.getStreamPrototype(s)+"]";
			str+=f.getName()+"="+f.getPrototype();
			if(f.getStopTree())
				str+="(L)";
		}
		str+="]"+centroid+"\n";*/
...
Map<Integer,ErrorStatistic>  estimateGetisAndOrdError(SnapshotData snap, int begin,int end){
		...
		for(Feature f:getSchema().getTargetList()){
				...
				error.put(f.getIndexMining(), new ErrorStatistic(f.getAutocorrelation(), end-begin+1));
		}
		
		return error;
	}
	
	Map<Integer,ErrorStatistic> estimateError(SnapshotData snap,  int begin,int end){
		...
		for(int i=begin;i<=end;i++){
	      	...
	      	for(Feature f: getSchema().getTargetList()){
	      		if(!snap.getSensorPoint(i).getMeasure(f.getIndexMining()).isNull()){
	      			error.get(f.getIndexMining()).countTuples++;
	      			Object predictedValue=predictedValueList.get(indexFeature);
	      			Object realValue=snap.getSensorPoint(i).getMeasure(f.getIndexMining()).getValue();
	      			if(f instanceof NumericFeature)
	      			{
	      				error.get(f.getIndexMining()).error+=Math.pow((Double)realValue-(Double)predictedValue, 2);
	      			}
	      			else{
	...
	void update(SnapshotData trainingSet,AutocorrelationI a, SnapshotWeigth W, 
		...
		this.schema.reset(); // resed all attributes excepted for leaf attributes
		...
		for(Feature f:this.getSchema().getTargetList()){
			 ...
			 else // in caso di foglia vado a prendere il modello dal padre (father)
			 {
		...
		for(Feature f:this.getSchema().getTargetList()){
				...
				if(!f.getStopTree())
					...
					if(father==null || !father.getSchema().getTargetList().get(index).getStopTree()) // non era già foglia
						f.computeAutocorrelation(a, trainingSet, W, beginExampleIndex, endExampleIndex);
				index++;
		}
