...
public class FromRToJava {
	...
	public HashMap<String, ForecastingModel> converter(
		...
		for (String combinations : hm.keySet()) {
			ArrayList<FeatureForecastingModel> model= new ArrayList<FeatureForecastingModel>(schema.getTargetList().size());
			...
			ArrayList<Double> coefTrend = (ArrayList<Double>) dataFromR.get(index.trend);
			ArrayList<Double> coefConst = (ArrayList<Double>) dataFromR.get(index.cost);
			
			for (Feature f : schema.getTargetList()) {
				int i=f.getFeatureIndex();
				FeatureForecastingModel VARModel = new FeatureVARForecastingModel(f,listOfCorrelatedFeature.get(i),listOfCorrelatedCoefficients.get(i),p,timeseries,coefConst.get(i),coefTrend.get(i));
