...
public class DynamicSnapshotWeight extends SnapshotWeigth{
	private Map<Integer,Map<Integer,Float>> D=new HashMap<Integer,Map<Integer,Float>>(); //triangular Superior matrix <idI-idJ, w>
		...
		//	System.out.println(i);
			...
			//	String key=data.getSensorPoint(i).getId()+":"+data.getSensorPoint(j).getId();
				...
				if(data.getSensorPoint(i).getId()!=data.getSensorPoint(j).getId())
					if(!keyI || !keyJ)
					{ // a new sensor in the current snapshot with respect to the past
						...
						if(d<=b)
						{
							...
							if(keyI){
								D.get(data.getSensorPoint(i).getId()).put(data.getSensorPoint(j).getId(),(float)d);
							}
	...
	public Double getWeight (Integer id1, Integer id2){
		...
		if(id1.intValue()==id2.intValue())
			...
			if(w==null) w=b; // approximate the distance with the boundary
			if(w.equals(0.0))
				return 1.0;
			else
