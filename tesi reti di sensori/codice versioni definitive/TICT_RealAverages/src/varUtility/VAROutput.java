...
public class VAROutput {
	...
	public static void printAndReset(SnapshotSchema schema, String idSnapshot,
		...
		/*
		 * for (VARParameter vp : RVar.getVARParameters()) { PrintStream stream
		 * = RMSEFilesOutput.get(vp.toString()); ArrayList<Double> rmse =
		 * counterRMSE.get(vp.toString()); stream.print("\n");
		 * stream.print("ID: " + idSnapshot + ";"); if (rmse!=null) { for
		 * (Double d : rmse) { double avg = d / leavesintree; String savg =
		 * String.valueOf(Double.valueOf(avg)); savg = savg.replace('.', ',');
		 * stream.print(savg + ";"); } } else { for (Feature f:
		 * schema.getTargetList()) { stream.print("null" + ";"); } } }
		 * 
		 * 
		 * 
		 * resetRMSE();
