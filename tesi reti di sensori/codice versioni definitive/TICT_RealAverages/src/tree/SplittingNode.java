...
public class SplittingNode extends Node {
	...
	public SplittingNode(SnapshotData trainingSet, SnapshotSchema schema,
		...
		setSplitInfo(new ResubstitutionIndex(), trainingSet, null,
	...
	public SplittingNode(AutocorrelationI a, SnapshotData trainingSet,
			...
			int endExampleIndex, int minExamples, int depth, int step,
			Node father, String testType) throws SplitException {
		super(a, trainingSet, schema, W, beginExampleIndex, endExampleIndex,
				minExamples, depth, father);

		setSplitInfo(a, trainingSet, W, beginExampleIndex, endExampleIndex,
				minExamples, step, testType);
	...
	public SplittingNode(Node n, AutocorrelationI a, SnapshotData trainingSet,
			SnapshotSchema schema, SnapshotWeigth W, int beginExampleIndex,
			int endExampleIndex, int minExamples, int depth, int step,
			Node father, String testType, int dim) throws SplitException {
	...
	public SplittingNode(Node n, AutocorrelationI a, SnapshotData trainingSet,
			SnapshotSchema schema, SnapshotWeigth W, int beginExampleIndex,
			int endExampleIndex, int minExamples, int depth, int step,
			Node father, String testType, FeatureWindow fAvgNode)
			throws SplitException {
		super(n, a, trainingSet, schema, W, beginExampleIndex, endExampleIndex,
				minExamples, depth, father);
	...
	public SplittingNode(Node n, AutocorrelationI a, SnapshotData trainingSet,
			...
			int endExampleIndex, int minExamples, int depth, int step,
			Node father, String testType, FeatureWindow fAvgNode,
			Network network, String substitutionWith) throws SplitException {
		super(n, a, trainingSet, schema, W, beginExampleIndex, endExampleIndex,
				minExamples, depth, father);
	...
	private void setSplitInfo(AutocorrelationI a, SnapshotData trainingSet,
			SnapshotWeigth W, int beginExampleIndex, int endExampleIndex,
			int minExample, int numberOfSplits, String testType)
			throws SplitException {
		...
		for (Feature fi : splitList) {
			...
			trainingSet.sort(fi, beginExampleIndex, endExampleIndex); // order
																		// by
																		// attribute
			int cDistinct = trainingSet.countDistinct(fi, beginExampleIndex,
					endExampleIndex);
			if (numberOfSplits > 0) {
			...
			double currentSplitValue = (Double) (trainingSet.getSensorPoint(
					beginExampleIndex).getMeasure(fi.getIndexMining())
					.getValue());
			for (int i = beginExampleIndex; i <= endExampleIndex; i++) {
				double value = (Double) (trainingSet.getSensorPoint(i)
					...
					for (Feature t : schema.getTargetListNotLeaf()) {
						if ((schema.getTargetList().size() > 1 && (t
							...
							resubstituitionGOErrorLeft.get(t.getIndexMining())
									.addX(trainingSet, beginExampleIndex,
											i - 1, W, t);
							resubstituitionGOErrorRight.get(t.getIndexMining())
									.subX(trainingSet, i + 1, endExampleIndex,
											W, t);
		...
		isLeaf = prepruning(trainingSet, W, beginExampleIndex, endExampleIndex,
				minExample, bestSplitPoint.midindex + 1);
	...
	private boolean prepruning(SnapshotData trainingSet, SnapshotWeigth W,
			int beginExampleIndex, int endExampleIndex, int minExample, int i) {
		boolean isLeaf = false;
		LeafNode left = new LeafNode(null, trainingSet, schema, W,
				beginExampleIndex, i - 1, minExample, getDepth() + 1, this);
		LeafNode right = new LeafNode(null, trainingSet, schema, W, i,
				endExampleIndex, minExample, getDepth() + 1, this);

		Map<Integer, ErrorStatistic> prunedE = this.estimateError(trainingSet,
				beginExampleIndex, endExampleIndex);

		Map<Integer, ErrorStatistic> leftE = left.estimateError(trainingSet,
				beginExampleIndex, i - 1);
		Map<Integer, ErrorStatistic> rightE = right.estimateError(trainingSet,
				i, endExampleIndex);
