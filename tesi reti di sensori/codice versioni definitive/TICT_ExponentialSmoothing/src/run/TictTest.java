...
public class TictTest {
	...
	public static void main(String[] args) throws IOException {
		...
		outputSmoothing = new PrintStream(new FileOutputStream(filesPath
				+ "output/stream/smoothing/" + dataName + "_" + "alfa" + alpha
				+ "smoothing.arff"));
		outputSmoothing.println("@");
		...
		try {
			...
			/*
			 * è lo snapshotData smutato si occupa di gestire lo snhapshot
			 * risultante dall'exp_smoothing
			 */
			SnapshotData snapSmoot = null;
					...
					while (true) {
						try {
							...
							if (snapData == null) {
								...
								snapForecast = snapData
										.initializeSnapshotForecast(schema);
							...
							GregorianCalendar timeAuxBegin= new GregorianCalendar();
							if (snapData.getIdSnapshot() == 2) {
								snapSmoot = snapData.computeSmoothing(null,
										null, null);
							} else {
								snapSmoot = snapData.computeSmoothing(alpha,
										snapSmoot, schema);
							}
							
							GregorianCalendar timeAuxEnd = new GregorianCalendar();
							long auxTreeTime=timeAuxEnd.getTimeInMillis()
									- timeAuxBegin.getTimeInMillis();

							/*
							 * stampa dello smoothing da eliminare in seguito
							 */
							for (SensorPoint sp : snapSmoot) {
								outputSmoothing.println(sp);
							}
							outputSmoothing.println("@");
							...
							if (!isSpatial.equals("GO")
								...
								snapSmoot.scaledSchema(schema);
							}

							if (autoCorrelation instanceof ResubstitutionIndexOnGetisOrd)
								snapSmoot.updateGetisAndOrd(W, schema);
							else {
								snapSmoot.scaledSchema(schema);
								snapData.scaledSchema(schema);
							}
							...
							if (tree == null)// first snapshot in the stream
							{
								...
								tree = new Tree(snapData, snapSmoot, schema, W,
								...
								tree.prune(snapData, snapSmoot, schema, W,
										autoCorrelation);
								afterPruningLeaves = tree.countLeaves();
								tree.drift(snapData, snapSmoot, schema, W,
										autoCorrelation, splitNumber, testType,
										network, substitutionWith);
								...
								/*
								 * if (snapData.getIdSnapshot() == 50) {
								 * 
								 * 
								 * for(int i = 0; i <=4; i++){ FeatureWindow fw
								 * =
								 * network.getFeatureWindow(snapData.getSensorPoint
								 * (i).getId()); for (TemporalWindow<Double> tw
								 * : fw) { PrintStream outputFile = new
								 * PrintStream( new FileOutputStream(
								 * "output/785/" + dataName +
								 * snapData.getSensorPoint(i).getId() +
								 * tw.getFeature() + ".csv")); for (Double d :
								 * tw) { String p = d.toString(); p =
								 * p.replace('.', ','); outputFile.println(p); }
								 * } } for(int i = 5; i <=9; i++){ FeatureWindow
								 * fw =
								 * network.getFeatureWindow(snapData.getSensorPoint
								 * (i).getId()); for (TemporalWindow<Double> tw
								 * : fw) { PrintStream outputFile = new
								 ...
								 * } } for(int i = 10; i <=15; i++){
								 * FeatureWindow fw =
								 * network.getFeatureWindow(snapData
								 * .getSensorPoint(i).getId()); for
								 * (TemporalWindow<Double> tw : fw) {
								 * PrintStream outputFile = new PrintStream( new
								 * FileOutputStream( "output/791/" + dataName +
								 * snapData.getSensorPoint(i).getId() +
								 * tw.getFeature() + ".csv")); for (Double d :
								 * tw) { String p = d.toString(); p =
								 * p.replace('.', ','); outputFile.println(p); }
								 ...
								 * (TemporalWindow<Double> tw : fw) {
								 * PrintStream outputFile = new PrintStream( new
								 * FileOutputStream( "output/793/" + dataName +
								 * snapData.getSensorPoint(i).getId() +
								 * tw.getFeature() + ".csv")); for (Double d :
								 * tw) { String p = d.toString(); p =
								 * p.replace('.', ','); outputFile.println(p); }
								 ...
								 * (TemporalWindow<Double> tw : fw) {
								 * PrintStream outputFile = new PrintStream( new
								 * FileOutputStream( "output/712/" + dataName +
								 * snapData.getSensorPoint(i).getId() +
								 * tw.getFeature() + ".csv")); for (Double d :
								 * tw) { String p = d.toString(); p =
								 * p.replace('.', ','); outputFile.println(p); }
								 * } } for(Node n : tree){ if(n instanceof
								 * LeafNode){
								 * 
								 * FeatureWindow fw
								 * =((LeafNode)n).getFeatureAvgNode();
								 * 
								 * 
								 * for (TemporalWindow<Double> tw : fw) {
								 * 
								 * PrintStream outputFile = new PrintStream( new
								 * FileOutputStream( "output" + "/cluster/" +
								 * n.getIdNode() + tw.getFeature() + ".csv"));
								 * for (Double d : tw) { String p =
								 * d.toString(); p = p.replace('.', ',');
								 * outputFile.println(p); } } }}
								 * tree.learnVARModel(rParameters); Forecasting
								 * forecast = new Forecasting( nahead, schema,
								 * tree, snapNetwork, network);
								 * LinkedList<SnapshotData> res = forecast
								 * .forecasting();
								 * outputForecastReport.addForecast(res);
								 * System.out.println(tree); return; }
								 */
