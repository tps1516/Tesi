...
public class ForecastingModel implements Iterable<FeatureForecastingModel>,
	...
	/*
	 * ha la responsabilità di memorizzare i modelli VAR di ciascuna FEATURE la
	 * posizione in cui il modello VAR viene memorizzato nell'arrayList
	 * corrisponde all'indice della feature stessa nello schema
	 */
	private ArrayList<FeatureForecastingModel> models;

	/*
	 * costruttore della classe ha la responsibilità di istanziare uno oggetto
	...
	public ForecastingModel(double[][] dataset, SnapshotSchema schema,
			ArrayList<Object> rParameters) throws NotForecastingException {
		try {
			...
			/*
			 * richiamo il metodo RForecasting e memorizzo i risultati ottenuti
			 * dalla chiamata nell'oggetto resultR tale oggetto è un hashmap
			 * tale che l'insieme delle chiavi rappresenta l'insieme di tutte le
			 * combinazioni possibili dei parametri di input al modello VAR
			 * mentre l elemento (arrayList<Object>) associato alla specifica
			...
			/*
			 * converto la struttura dati ottenuta dalla chiamata precedente in
			 * una struttura analoga che però memorizza per ciascuna chiave un
			 * oggetto ForecastingModel ogni oggetto corrisponde ad una
			 * specifica combinazione di parametri
			 */
			...
			/*
			 * costruisco l'oggetto forecastingmodel finale dove il modello var
			 * per ciascuna feature corrisponde al modello costruito con la
			 * combinazione di parametri che minimizza il valore del RMSE
			 */
			setModels(opt.computeOptimalVARModel(resultRToJava, schema));
		} catch (NotForecastingException e) {
			throw new NotForecastingException();
		}
	}

	/*
	 * costruttore della classe setta l'attributo models con l'arrayList che
	 * arriva in input tale costruttore viene utilizzato per costruire un
	 * oggetto forecastingmodel dove i modelli VAR per ciascuna feature sono
	 * costruiti tutti conl la stessa combinazione di parametri
	 */
	...
	/*
	 * restituisca il modello VAR per la specifica feature che viene passata in
	 * input
	 */
	...
	/*
	 * metodo privato setta l'attributo models
	 */
	...
	private void checkLearnVARModels(double[][] matrix)
